...
with graph.as_default():
    ...
    with tf.device('/gpu:0'):
        with sess.as_default():
            ...
            intermediate_layer_model = keras.models.Model(inputs=model.input, outputs=model.get_layer("predictores").output)

            for i in range(len(X_test)):
                X_train[i] = intermediate_layer_model.predict(X_train[i], batch_size=128)
                X_test[i] = intermediate_layer_model.predict(X_test[i], batch_size=128)
