import Redes.Red_LSTM_Conv3D as Interface
        ...
        with tf.device('/gpu:'+str(index_gpu)):
            with sess.as_default():
                ...
                history, model = Interface.Red.train(model, index_data, name_path + '_all_'+str(index_data), X_train[index_data], y_train[index_data], X_test[index_data],
                                         y_test[index_data], noise=1, l2_noise=1, weight_decay_noise=0.00001,  stepped = True,  class_weight = {0 : 0.25, 1 : 0.75})
...
name_path = "../Results/result_05_fullWindow/lstm_conv3d_4_capas/escalado/con_weight_0.75/"
...
for i in range(number_folders):
    X_train[i] = np.expand_dims(X_train[i], axis=5)
    X_test[i] = np.expand_dims(X_test[i], axis=5)
