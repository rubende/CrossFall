...
[data_all, labels_all, subjects_all, ages_all] = Raw.createDataset_11("../Raw/BaseDatos_011_SisFall_Colombia.mat")
...
[data_all, labels_all, subjects_all, ages_all] = Data.dataAugmentation_11(data_all, labels_all, subjects_all, ages_all)
    ...
    with tf.device('/gpu:0'):
        with sess.as_default():
            ...
            intermediate_layer_model = keras.models.Model(inputs=model.input,
                                                          outputs=model.get_layer("predictores").output)

            for i in range(len(data_all)):
                predictors.append(intermediate_layer_model.predict(np.expand_dims(data_all[i],0), batch_size=128))


with open("../Data/data_11/"+'predictores_normalizadoEstandarizado_subjects_age.pkl', 'wb') as f:  # Python 3: open(..., 'wb')
    pickle.dump([data_all, labels_all, subjects_all, ages_all, predictors], f)
