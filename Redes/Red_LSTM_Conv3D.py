...
class Red:
    @staticmethod
    def build(input_shape, num_classes, number_convolutional_layers, first_number_filters, weight_decay=1e-4,
        ...
        model_input = Input(shape=(6, 1, 50, 3, 1))
        print(tf.shape(model_input))
        output = TimeDistributed(Conv3D(first_number_filters, kernel_size=(1, filter_size, 1), kernel_regularizer=L2_norm, \
                        activation='relu', input_shape=(6,1,50,3)))(model_input)
        output = TimeDistributed(BatchNormalization())(output)
        output = TimeDistributed(MaxPooling3D(pool_size=(1, 2, 1)))(output)
        ...
        for i in range(1, number_convolutional_layers):
            ...
            # model.summary()
        print(tf.shape(output))
        print(output.shape[0].value)
        print(output.shape[1].value)
        print(output.shape[2].value)
        print(output.shape[3].value)
        print(output.shape[4].value)

        output = TimeDistributed(AveragePooling3D(pool_size=(1, output.shape[3].value, output.shape[4].value)))(output)
    ...
    def train(model_defined, index_folder, path_file_last, x_train, y_train, x_test, y_test, batch_size=128, ADAM=0, save_name=None, noise=0,
        ...
        if not stepped:
            if ADAM == 0:
                history = model_defined.fit(x_train, y_train,
            ...
            for i in range(math.ceil(len(t3)/t2)):
                ...
                history = model_defined.fit(x_train[index_temp], y_train[index_temp],
