...
class Red:
    ...
    def build(input_shape_1, input_shape_2, num_classes, number_convolutional_layers, first_number_filters, weight_decay=1e-4,
        ...
        print("Debug")
        print("1 "+str(model_input_1.shape))
        ...
        print("2" +str(output_1.shape))
        output_1 = BatchNormalization()(output_1)
        print("3" +str(output_1.shape))
        avg_1 = AveragePooling2D(pool_size=(1, output_1.shape[2].value))(output_1)
        print("4" +str(avg_1.shape))
        fc_1 = Flatten()(avg_1)
        fc_1 = Dense(num_classes, activation='softmax', name = 'fc_1')(fc_1)
        print("5" + str(fc_1.shape))
        print("Debug")
        # BRANCH 2
        ...
        avg_2 = AveragePooling2D(pool_size=(1, output_2.shape[2].value))(output_2)
        fc_2 = Flatten()(avg_2)
        fc_2 = Dense(num_classes, activation='softmax', name = 'fc_2')(fc_2)

        output = concatenate([output_1, output_2])
        print("6" + str(output.shape))
        ...
        output = Dense(num_classes, activation='softmax', name = 'dense_output')(output)


        model = Model(inputs=[model_input_1,model_input_2], outputs=[output, fc_1, fc_2])
        model.summary()
        return model
    ...
    def train(model_defined, index_folder, path_file_last, x_train, y_train, x_test, y_test, batch_size=128, ADAM=0, save_name=None, noise=0,
        ...
        losses = dict()
        losses['fc_1'] = 'categorical_crossentropy'         # 301
        losses['fc_2'] = 'categorical_crossentropy'            # 51
        losses['dense_output'] = 'categorical_crossentropy'

        losses_w = [0.3, 0.7, 1.0]
        ...
        if noise == 0:
            model_defined.compile(loss=losses, optimizer=optimizer, loss_weights= losses_w,
            ...
            model_defined.compile(loss=losses, loss_weights= losses_w,
        ...
        if not stepped:
            if ADAM == 0:
                history = model_defined.fit([x_train, x_train[:,:, :, 125:176, :]], [y_train, y_train, y_train],
                                            batch_size=batch_size, epochs=100,
                                            verbose=0, validation_data=([x_test, x_test[:, :, 125:176, :]], [y_test, y_test ,y_test]),
            ...
            for i in range(math.ceil(len(t3)/t2)):
                ...
                history = model_defined.fit([x_train[index_temp], x_train[index_temp][:, :, 125:176, :]], [y_train[index_temp], y_train[index_temp], y_train[index_temp]],
